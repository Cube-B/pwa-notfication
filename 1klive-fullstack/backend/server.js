...
// PWA Configuration Routes (Public - no auth required)
...
// ========================================
app.post('/api/notifications/update-carrier', async (req, res) => {
  try {
    const { 
      endpoint, 
      carrier, 
    ...
    console.log('📡 Updating carrier info:', {
      carrier,
      device_type,
      is_mobile_network,
    ...
    const checkResult = await pool.query(
      'SELECT has_mobile_carrier FROM subscriptions WHERE endpoint = $1',
      [endpoint]
    );
    ...
    // If already has mobile carrier, skip update
    if (checkResult.rows[0].has_mobile_carrier) {
      console.log('✅ Already has mobile carrier, skipping update');
      return res.json({
        success: true,
        message: 'Already has mobile carrier data',
        skip: true
      });
    }
    ...
    const updateQuery = `
      ...
      SET 
        ...
        is_mobile_network = $5,
        detection_method = $6,
        last_carrier_check = CURRENT_TIMESTAMP,
        has_mobile_carrier = $7,
        carrier_detected_at = CASE 
          WHEN $7 = true AND carrier_detected_at IS NULL 
          THEN CURRENT_TIMESTAMP 
          ELSE carrier_detected_at 
        END
      WHERE endpoint = $8
    `;
    ...
    await pool.query(updateQuery, [
      ...
      is_mobile_network,
      detection_method,
      is_mobile_network, // has_mobile_carrier = true if is_mobile_network
      endpoint
    ]);
    
    console.log('✅ Carrier info updated');
    
    res.json({
      success: true,
      message: 'Carrier info updated',
      is_mobile: is_mobile_network,
      carrier: carrier
    });
...
// ========================================
// 📊 Check if has mobile carrier
// ========================================
app.post('/api/notifications/check-carrier', async (req, res) => {
  try {
    ...
    res.json({
      success: true,
      has_mobile_carrier: data.has_mobile_carrier || false,
      should_collect: !data.has_mobile_carrier,
