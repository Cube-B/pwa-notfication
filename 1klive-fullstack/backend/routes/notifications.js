  ...
  try {
    ...
    const logResult = await pool.query(`
      INSERT INTO notification_logs 
        (title, message, url, target_type, target_filter, total_recipients, status, created_by)
    ...
    `, [
      ...
      url || null,
      target || 'all',
      JSON.stringify({ carriers, devices }),
...
router.post('/broadcast', async (req, res) => {
  try {
    ...
    const logResult = await pool.query(`
      INSERT INTO notification_logs 
        (title, message, url, target_type, total_recipients, status, created_by)
      VALUES ($1, $2, $3, 'broadcast', $4, 'pending', $5)
      RETURNING id
    `, [title, message, url || null, subscriptions.length, adminId]);
...
router.get('/templates', async (req, res) => {
  try {
    ...
    const result = await pool.query(`
      SELECT 
        id, name, title, message, url, category,
...
router.post('/templates', async (req, res) => {
  try {
    const { name, title, message, url, category } = req.body;
    const adminId = req.user.id;
    ...
    const result = await pool.query(`
      INSERT INTO notification_templates 
        (name, title, message, url, category, created_by)
      VALUES ($1, $2, $3, $4, $5, $6)
      RETURNING *
    `, [name, title, message, url || null, category || 'general', adminId]);
